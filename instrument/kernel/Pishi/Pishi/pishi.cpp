...
#define PISNI_PATH "/dev/" PISHI_DEVNODE
...
IOBufferMemoryDescriptor * createSharedMemory(size_t size) {
    ...
    memoryDescriptor = IOBufferMemoryDescriptor::withOptions(kIODirectionOutIn | kIOMemoryKernelUserShared, size, 4096);
...
uintptr_t* map_memoryinto_current_task()
{
    ...
    memset(buffer_instrument, 0x41, 8);
...
pishi_ioctl(dev_t dev, unsigned long cmd, caddr_t _data, int fflag, proc_t p)
{
    ...
    switch (cmd) {
        ...
        case PISHI_IOC_MAP:
        {
            ...
            uint32_t high = ((p->ptr  ) >> 32) & 0xFFFFFFFF;
            uint32_t low = (p->ptr  ) & 0xFFFFFFFF;
            my_printf("[meysam] PISHI_IOC_MAP IOCTL memory maped into 0x%08X%08X\n", high,low);
            my_printf("[meysam:PISHI_IOC_MAP] instrumented_thread %llu, do_instrument %d" ,instrumented_thread, do_instrument);
            break;
        }
        case PISHI_IOC_START:
        {
            // TODO: atomic
            if (do_instrument)
            {
                my_printf("[meysm] PISHI_IOC_START do_instrument is already on\n");
                return 0;
            }
            ...
            do_instrument = true;
            my_printf("[meysam:PISHI_IOC_START] instrumented_thread %llu, do_instrument %d\n" ,instrumented_thread, do_instrument);
            kcov* area = (kcov*) buffer_instrument;
            if(area)
                area->kcov_pos = 0;
            break;
        ...
        case PISHI_IOC_STOP:
        {
            ...
            my_printf("[meysam:PISI_IOC_STOP] instrumented_thread %llu, do_instrument %d\n" ,instrumented_thread, do_instrument);
            break;
        }
            
        case PISHI_IOC_TEST:
        {
            // generate hits, to test full package, otherwise we have to wait until ghidra finishes its job.
            sanitizer_cov_trace_pc(0x4141414141);
            my_printf("[meysam:PISI_IOC_TEST] instrumented_thread %llu, do_instrument %d\n" ,instrumented_thread, do_instrument);
            break;
        }
        ...
        case PISHI_IOC_UNMAP:
        {
            my_printf("[meysam:PISI_IOC_UNMAP] instrumented_thread %llu, do_instrument %d\n" ,instrumented_thread, do_instrument);
...
void instrument_thunks()
{
    asm volatile (
                  ...
                  "    STR x30, [sp, #-16]!\n"// save LR. we can't restore it in pop_regs, otherwise it will restore the ret  of original block as
                  "    bl _push_regs\n"                                                          // return value of pop_regs
                  "    mov x0, #0x4141\n"     // move value into x0 (note the correction)
                  "    mov x0, #0x4141\n"
                  "    mov x0, #0x4141\n"
                  "    mov x0, #0x4141\n"
